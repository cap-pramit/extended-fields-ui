...
const CUSTOM_APP_TYPE = "custom";
const EXTERNAL_APP_TYPE = "external"
const SUPPORTED_APP_TYPES = [NATIVE_APP_TYPE, CUSTOM_APP_TYPE, EXTERNAL_APP_TYPE];
...
const setup = async (appName, projectDir) => {
  try {
    ...
    if (fs.existsSync(configPath)) {
      console.log(chalk.green('Configuration already exists!, You can still modify certain fields.'));
    ...
    const answers = await inquirer.prompt([
      ...
      },
      {
        type: 'input',
        name: 'appType',
        message: `Is this a ${NATIVE_APP_TYPE} OR ${CUSTOM_APP_TYPE} or ${EXTERNAL_APP_TYPE} app? (This selection cannot be changed once app is created)`,
        default: existingConfig.appType || CUSTOM_APP_TYPE,
      ...
      },
      {
        type: 'input',
        name: 'bugsnagApiKey',
        message: 'Enter your Bugsnag API key:',
      ...
      },
      {
        type: 'confirm',
        name: 'useSourceMaps',
        message: 'Do you want to use sourcemaps?',
        default: false,
        when: (answers) => !answers.useBugsnag,
      },
      {
        type: 'confirm',
        name: 'useI18n',
        message: 'Do you want to use i18n translations?',
        default: false,
      },
      {
        type: 'confirm',
        name: 'customI18n',
        message: 'Since this is a custom app, do you want to provide locale details for i18n?',
        default: false,
        when: (answers) => answers.useI18n && (answers.appType || existingConfig.appType) === CUSTOM_APP_TYPE,
      },
      {
        type: 'input',
        name: 'locales',
      ...
      },
      {
        type: 'input',
        name: 'gtmTrackingId',
        message: 'Enter your Google Tag Manager Tracking ID:',
      ...
      },
      {
        ...
        message: 'Do you want to use Navigation Component in your project?',
        default: false,
        when: (answers) => (answers.appType !== EXTERNAL_APP_TYPE || existingConfig.appType !== EXTERNAL_APP_TYPE),
      }
    ]);
